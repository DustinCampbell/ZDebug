...
internal sealed class OutputViewModel : ViewModelWithViewBase<UserControl>, IScreen, ISoundEngine
{
    ...
    protected override void ViewCreated(UserControl view)
    {
        ...
        windowContainer = View.FindName<Grid>("windowContainer");
    }

    private void storyService_StoryOpened(object sender, StoryOpenedEventArgs e) => throw new NotImplementedException();
    ...
    private FormattedText GetFixedFontMeasureText()
    {
        return new FormattedText(
            ...
            pixelsPerDip: View.GetPixelsPerDip());
    }

    private bool ForceFixedWidthFont() =>
        // TODO: Move into appropriate API
        (storyService.Story.Memory.ReadWord(0x10) & 0x02) == 0x02;
    ...
    private void ResetStatusHeight()
    {
        if (upperWindow != null)
        {
            var height = upperWindow.GetHeight();
    ...
    public void Print(string text)
    {
        ...
        {
            var oldValue = windowManager.ActiveWindow.SetFixedPitch(true);
    ...
    public void ReadCommand(int maxChars, Action<string> callback)
    {
        if (gameScriptService.HasNextCommand())
        {
            ...
            var command = gameScriptService.GetNextCommand();
    ...
    public int GetCursorLine() => windowManager.ActiveWindow.GetCursorLine();

    public int GetCursorColumn() => windowManager.ActiveWindow.GetCursorColumn();

    public void SetCursor(int line, int column) => windowManager.ActiveWindow.SetCursor(column, line);
    ...
    public void HighBeep() => SystemSounds.Asterisk.Play();

    public void LowBeep() => SystemSounds.Beep.Play();
}
