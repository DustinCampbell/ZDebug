...
public class CompiledRunner : Runner, IZMachineProfiler
{
    ...
    void IZMachineProfiler.EnterRoutine(int address) => callCount++;
    ...
    void IZMachineProfiler.ExecutedInstruction(int address) => instructionCount++;

    void IZMachineProfiler.Quit() => watch.Stop();

    void IZMachineProfiler.Interrupt() => watch.Stop();
}
