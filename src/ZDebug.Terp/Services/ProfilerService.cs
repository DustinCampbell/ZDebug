using System;
using System.Composition;
using System.Diagnostics;
using System.Linq;
using ZDebug.Terp.Profiling;
using ZDebug.UI.Services;

namespace ZDebug.Terp.Services;

[Export, Shared]
internal sealed class ProfilerService : IService
{
    private ZMachineProfiler profiler;
    private Stopwatch watch;

    private TimeSpan compileTime;
    private int routinesCompiled;
    private double zcodeToILRatio;
    private int routinesExecuted;
    private int instructionsExecuted;
    private int calculatedVariableLoads;
    private int calculatedVariableStores;
    private int directCalls;
    private int calculatedCalls;

    [ImportingConstructor]
    public ProfilerService()
    {
    }

    private void OnStarting()
    {
        var handler = Starting;
        if (handler != null)
        {
            handler(this, new ProfilerStartingEventArgs());
        }
    }

    private void OnStopped()
    {
        var handler = Stopped;
        if (handler != null)
        {
            handler(this, new ProfilerStoppedEventArgs());
        }
    }

    public void Create() => profiler = new ZMachineProfiler();

    public void Destroy() => profiler = null;

    public void Start()
    {
        OnStarting();
        watch = Stopwatch.StartNew();
    }

    public void Stop()
    {
        watch.Stop();
        if (profiler != null)
        {
            profiler.Stop(watch.Elapsed);
        }
        OnStopped();
    }

    public void UpdateProfilerStatistics()
    {
        if (profiler == null)
        {
            return;
        }

        var compilationStatistics = profiler.CompilationStatistics.ToList();
        if (compilationStatistics.Count > 0)
        {
            compileTime = new TimeSpan(compilationStatistics.Sum(s => s.CompileTime.Ticks));
            routinesCompiled = profiler.RoutinesCompiled;
            zcodeToILRatio = compilationStatistics.Select(s => (double)s.Size / (double)s.Routine.Length).Average();
            routinesExecuted = profiler.RoutinesExecuted;
            instructionsExecuted = profiler.InstructionsExecuted;
            calculatedVariableLoads = compilationStatistics.Sum(s => s.CalculatedLoadVariableCount);
            calculatedVariableStores = compilationStatistics.Sum(s => s.CalculatedStoreVariableCount);
            directCalls = profiler.DirectCallCount;
            calculatedCalls = profiler.CalculatedCallCount;
        }
        else
        {
            compileTime = TimeSpan.Zero;
            routinesCompiled = 0;
            zcodeToILRatio = 0;
            routinesCompiled = 0;
            instructionsExecuted = 0;
            calculatedVariableLoads = 0;
            calculatedVariableStores = 0;
            directCalls = 0;
            calculatedCalls = 0;
        }
    }

    public bool Profiling => profiler != null;

    public TimeSpan Elapsed => watch.Elapsed;

    public ZMachineProfiler Profiler => profiler;

    public TimeSpan CompileTime => compileTime;

    public int RoutinesCompiled => routinesCompiled;

    public double ZCodeToILRatio => zcodeToILRatio;

    public double ZCodeToILRatioPercent => zcodeToILRatio * 100;

    public int RoutinesExecuted => routinesExecuted;

    public int InstructionsExecuted => instructionsExecuted;

    public int CalculatedVariableLoads => calculatedVariableLoads;

    public int CalculatedVariableStores => calculatedVariableStores;

    public int DirectCalls => directCalls;

    public int CalculatedCalls => calculatedCalls;

    public event EventHandler<ProfilerStartingEventArgs> Starting;
    public event EventHandler<ProfilerStoppedEventArgs> Stopped;
}
