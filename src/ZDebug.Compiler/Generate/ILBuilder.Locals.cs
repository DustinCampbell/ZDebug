...
public sealed partial class ILBuilder
{
    private class LocalWrapper : ILocal
    {
        ...
        public void Renew() => released = false;
    ...
    private class ArrayLocalWrapper : LocalWrapper, IArrayLocal
    {
        ...
        public ArrayLocalWrapper(ILBuilder builder, LocalBuilder local)
            : base(builder, local)
        {
            elementType = local.LocalType.GetElementType();
    ...
    private class RefLocalWrapper : LocalWrapper, IRefLocal
    {
        ...
        public void LoadIndirectValue() => builder.Emit(loadOpCode);
        ...
        public void StoreIndirectValue() => builder.Emit(storeOpCode);
    }

    private static readonly Func<ILBuilder, LocalBuilder, LocalWrapper> CreateLocal = (l, b) => new LocalWrapper(l, b);
    private static readonly Func<ILBuilder, LocalBuilder, ArrayLocalWrapper> CreateArrayLocal = (l, b) => new ArrayLocalWrapper(l, b);
    private static readonly Func<ILBuilder, LocalBuilder, RefLocalWrapper> CreateRefLocal = (l, b) => new RefLocalWrapper(l, b);

    private TWrapper AllocateLocal<T, TWrapper>(Func<ILBuilder, LocalBuilder, TWrapper> createWrapper, bool byref = false) where TWrapper : LocalWrapper => AllocateLocal<TWrapper>(typeof(T), createWrapper, byref);
    ...
    public ILocal NewLocal<T>() => AllocateLocal<T, LocalWrapper>(CreateLocal);

    public ILocal NewLocal(Type type) => AllocateLocal<LocalWrapper>(type, CreateLocal);
    ...
    public IArrayLocal NewArrayLocal<T>() => AllocateLocal<T[], ArrayLocalWrapper>(CreateArrayLocal);
    ...
    public IRefLocal NewRefLocal<T>() => AllocateLocal<T, RefLocalWrapper>(CreateRefLocal, byref: true);
