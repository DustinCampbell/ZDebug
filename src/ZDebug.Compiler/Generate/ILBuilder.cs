...
public sealed partial class ILBuilder
{
    ...
    private readonly Dictionary<Type, Stack<ILocal>> locals = [];
    ...
    private static readonly MethodInfo arrayCopy = Reflection<Array>.GetMethod(
        "Copy",
        Types.Array<Array, Array, int>(),
        instance: false);

    private static readonly MethodInfo mathMin = typeof(Math).GetMethod(
        name: "Min",
    ...
    private static readonly ConstructorInfo exceptionCtor = Reflection<ZMachineException>.GetConstructor(Types.Array<string>());

    public ILBuilder(ILGenerator il)
    {
        this.il = il;

        arguments = new ArgumentFunctions(this);
    ...
    public void NewObject(ConstructorInfo constructor) => Emit(OpCodes.Newobj, constructor);
