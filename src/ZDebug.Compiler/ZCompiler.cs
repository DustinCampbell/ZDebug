...
internal partial class ZCompiler
{
    ...
    public ZCompilerResult Compile()
    {
        ...
        il = new ILBuilder(dm.GetILGenerator());

        calls = [];

        Profiler_EnterRoutine();

        controlFlowGraph = ControlFlowGraph.Build(routine);
        addressToLabelMap = new Dictionary<int, ILabel>(controlFlowGraph.CodeBlocks.Count());

        // Determine whether stack, memory, screen and outputStreams are used.
        foreach (var codeBlock in controlFlowGraph.CodeBlocks)
        {
            addressToLabelMap.Add(codeBlock.Address, il.NewLabel());

            foreach (var i in codeBlock.Instructions)
            {
                if (!usesStack && i.UsesStack())
                {
                    usesStack = true;
                }

                if (!usesMemory && i.UsesMemory())
                {
                    usesMemory = true;
                }
            }
        ...
        var instructionStatistics = new List<InstructionStatistics>(routine.Instructions.Length);
        ...
        foreach (var codeBlock in controlFlowGraph.CodeBlocks)
        {
            ...
            {
                if (addressToLabelMap.TryGetValue(generator.Instruction.Address, out var label))
                {
                    label.Mark();
                }
        ...
        return new ZCompilerResult(routine, calls.ToArray(), code, statistics);
    }

    private static void Optimize(List<OpcodeGenerator> generators)
    {
        for (var i = 0; i < generators.Count - 1; i++)
        {
    ...
    public static ZCompilerResult Compile(ZRoutine routine, CompiledZMachine machine) => new ZCompiler(routine, machine).Compile();
}
