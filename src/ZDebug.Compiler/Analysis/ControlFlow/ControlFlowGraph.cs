...
internal partial class ControlFlowGraph
{
    ...
    private ControlFlowGraph(ZRoutine routine)
    {
        ...
        entry = new Block(isEntry: true);
        exit = new Block(isExit: true);

        var instructions = new InstructionLinkedList(routine);
        codeBlocks = [.. BuildGraph(instructions)];
    }
    ...
    private List<CodeBlock> BuildGraph(InstructionLinkedList instructions)
    {
        ...
        entry.AddJumpTarget(currentBlock);
    ...
    public static ControlFlowGraph Build(ZRoutine routine) => new ControlFlowGraph(routine);
}
