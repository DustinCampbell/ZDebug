...
public sealed class InstructionCache
{
    ...
    internal bool TryGet(int address, out Instruction instruction) => map.TryGetValue(address, out instruction);

    internal void Add(int address, Instruction instruction) => map.Add(address, instruction);
