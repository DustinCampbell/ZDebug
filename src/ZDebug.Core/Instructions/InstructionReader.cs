...
public sealed class InstructionReader
{
    ...
    private readonly byte[] operandKinds = new byte[8];

    public InstructionReader(int address, byte[] memory, InstructionCache cache = null)
    {
        this.memory = memory;
        this.address = address;
        opcodeTable = OpcodeTables.GetOpcodeTable(memory[0]);
    ...
    {
        var size = 8;
        for (var i = 0; i < operandKinds.Length; i++)
        {
    ...
    private Variable ReadVariable(ref int address) => Variable.FromByte(memory.ReadByte(ref address));
    ...
    private static byte LongForm(byte opByte) => (byte)(opByte & 0x1f);

    private static byte ShortForm(byte opByte) => (byte)(opByte & 0x0f);

    private static byte VarForm(byte opByte) => (byte)(opByte & 0x1f);

    public Instruction NextInstruction()
    {
        var startAddress = address;

        if (cache.TryGet(startAddress, out var instruction))
        {
