...
{
    [Conditional("STRICT")]
    private static void Fail(Instruction instruction, string message) => throw new StrictException(instruction, message);

    [Conditional("STRICT")]
    private static void Fail(Instruction instruction, string format, params object[] args) => Fail(instruction, string.Format(format, args));
