...
public sealed class Story
{
    ...
    private Story(byte[] memory)
    {
        ...
        serialNumber = Header.ReadSerialNumber(memory);
        releaseNumber = Header.ReadReleaseNumber(memory);
        checksum = Header.ReadChecksum(memory);
        actualChecksum = Header.CalculateChecksum(memory);
        routinesOffset = Header.ReadRoutinesOffset(memory);
        stringsOffset = Header.ReadStringsOffset(memory);
        instructionCache = new InstructionCache((memory.Length - Header.ReadStaticMemoryBase(memory)) / 8);
        ztext = new ZText(memory);
        memoryMap = new MemoryMap(memory);
        informData = new InformData(memory, memoryMap, ztext);
        objectTable = new ZObjectTable(memory, ztext);
        globalVariablesTable = new GlobalVariablesTable(memory);
        dictionary = new ZDictionary(this, ztext);
        mainRoutineAddress = Header.ReadMainRoutineAddress(memory);
    ...
    public void RegisterInterpreter(IInterpreter interpreter)
    {
        ...
        // Set various flags
        var flags1 = memory.ReadByte(0x01);
        if (version <= 3)
        {
    ...
    public ZText ZText => ztext;

    public byte Version => version;

    public int SerialNumber => serialNumber;

    public ushort ReleaseNumber => releaseNumber;

    public ushort Checksum => checksum;

    public ushort ActualChecksum => actualChecksum;

    public MemoryMap MemoryMap => memoryMap;

    public bool IsInformStory => Header.IsInformStory(memory);

    public InformData InformData => informData;

    public ZObjectTable ObjectTable => objectTable;

    public GlobalVariablesTable GlobalVariablesTable => globalVariablesTable;

    public ZDictionary Dictionary => dictionary;

    public int MainRoutineAddress => mainRoutineAddress;

    public static Story FromBytes(byte[] bytes) => new Story(bytes);

    public static Story FromStream(Stream stream) => new Story(stream.ReadFully());
}
