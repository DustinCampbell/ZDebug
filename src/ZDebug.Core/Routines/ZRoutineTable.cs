...
public sealed class ZRoutineTable : IIndexedEnumerable<ZRoutine>
{
    ...
    public ZRoutineTable(Story story, InstructionCache cache = null)
    {
        ...
        routines = new IntegerMap<ZRoutine>(8192);
        sortedAddresses = [];
    ...
    private int UnpackCallAddress(Instruction i) => story.UnpackRoutineAddress(i.Operands[0].Value);
    ...
    public bool Exists(int address) => routines.Contains(address);

    public ZRoutine GetByAddress(int address) => routines[address];

    public bool TryGetByAddress(int address, out ZRoutine routine) => routines.TryGetValue(address, out routine);

    public ZRoutine this[int index] => routines[sortedAddresses[index]];
