...
public sealed class ZText
{
    ...
    {
        this.memory = memory;
        alphabetTable = new AlphabetTable(memory);
        version = Header.ReadVersion(memory);
        builder = new StringBuilder();
        abbreviationAlphabetTable = new AlphabetTable(memory);
        abbreviationBuilder = new StringBuilder();
    }
    ...
    public string ZWordsAsString(ushort[] zwords, ZTextFlags flags) => ZWordsAsString(zwords, flags, alphabetTable, builder);
    ...
    private ushort[] EncodeZText(string text)
    {
        ...
        var zchars = new byte[resolution * 3];
    ...
    {
        var tokenMax = bytes[parse++];
        var tokenCount = bytes[parse];
        ...
        var address = LookupWord(word, dictionary);
    ...
    public ushort LookupWord(string word, int dictionaryAddress)
    {
        ...
        var entryLength = memory.ReadByte(ref dictionaryAddress);
        var entryCount = memory.ReadWord(ref dictionaryAddress);
        ...
        ushort lower = 0;
        var upper = (ushort)(entryCount - 1);

        while (lower <= upper)
        {
            var entryNumber = sorted
                ? (ushort)((lower + upper) / 2)
                : lower;

            var entryAddress = (ushort)(dictionaryAddress + (entryNumber * entryLength));

            var addr = entryAddress;

            var cont = false;
            for (var i = 0; i < resolution; i++)
            {
                var entry = memory.ReadWord(addr);
